...
class MemeStocksStrategy(QCAlgorithm):
   ...
   Developer Notes (12/18/2025):
    ...
    Developer Notes (12/20/2025):
    - ✅ Uses data.OptionChains.Values — iterates through all chains that have new data this tick.
    - ✅ Gets chain.Symbol.Underlying — correctly extracts the equity symbol: Replaced OptionChain() calls with data.OptionChains access in OnData(), stopped using self.OptionChain() (singular) to manually pull data AFTER the universe is created.
    - ✅ Filters for c.Right == OptionRight.Call and .Put — gets both legs.
    - ✅ Uses atm_call.ImpliedVolatility — reads IV directly from the contract object (not an indicator).
    - ✅ Stores in self.iv_cache — every tick, fresh IV is cached.
    Developer Notes (12/21/2025):
    - Removed verbose debug self.logs (Universe selection, IV setup, Screening debugs) to reduce noise.
    - Commented out some logs to reuse later if needed.

    Problems::::
    The Problem We Had:
Using self.OptionChain(symbol) — This is a manual, asynchronous method that does NOT guarantee data is available

It doesn't respect the data slice from OnData()

It tries to pull data independently, which fails in backtesting and paper trading

It's meant for specific, one-off queries, not streaming option data

Accessing data.OptionChains in OnData() — This is the canonical, synchronous approach

The data object contains only contracts that have new quotes this tick

It's guaranteed to be in sync with the simulation clock

It's the same pattern used in the working spy-options.py

The Namespace/Causal Issue
You're also right about a causal namespace issue:

We were trying to create indicators (self.IV()) in a scheduled event (SelectContractsAndCreateIndicators) that ran independently

Then trying to read those indicators in OnData(), but the indicators were never actually receiving data because they were never subscribed to the right data source

The proper data flow is: data.OptionChains → directly extract IV → cache it, all within OnData()
    Developer Notes (12/22/2025):
    - Removed `MIN_IMPLIED_VOLATILITY` as it is no longer used.
    - Order of Operations for Screening:
      1. **Universe Selection**: Top 75 stocks by DollarVolume ($10M+) with price $2.50-$350.
      2. **Option Universe**: Front month contracts, +/- 3 strikes.
      3. **IV Data Collection (OnData)**: Caches ATM Call IV for active universe symbols.
      4. **Weekly Screening Gates**:
         - Schedule (Mondays).
         - IV Cache Readiness (>= 20 stocks).
         - Frequency (14 days since last trade).
         - Market Regime (VIX < 20.50).
      5. **Candidate Filtering**:
         - Active Equity Universe check.
         - IV Growth: Absolute IV (first run) or Positive IV Increase (subsequent runs).
         - Sentiment: Call/Put Volume Ratio >= 1.10.
      6. **Execution**: Top 15 candidates sorted by IV metric, 2% allocation each.
    """
    ...
    def Initialize(self):
        ...
        self.last_trade_execution_date = None # Track actual trade execution for 14-day cadence
        self.screening_frequency_days = 14  # Every 2 weeks (change to 21 for 3 weeks)
        self.initial_screen_done = False # One-shot screening flag
        self.screening_completed_today = False # Flag to prevent OnData loop
        ...
        # Chain an options universe to the equity universe, filtering for front-month contracts +/- 3 strikes
    ...
    def OptionFilterFunction(self, option_filter_universe: OptionFilterUniverse) -> OptionFilterUniverse:
        ...
        return option_filter_universe.Strikes(-3, +3).FrontMonth()
    ...
    def UniverseSelectionFunction(self, coarse: List[CoarseFundamental]) -> List[Symbol]:
        ...
        top_75 = sorted(filtered, key=lambda c: c.DollarVolume, reverse=True)[:75]
        
        return [c.Symbol for c in top_75]
    ...
    def OnSecuritiesChanged(self, changes: SecurityChanges):
        ...
        for security in changes.RemovedSecurities:
            if security.Symbol in self.iv_cache:
                del self.iv_cache[security.Symbol]
    ...
    def OnData(self, data):
        ...
        # Read fresh IV every tick
        self.ReadIVFromData(data)
        
        # === REVIEW THIS: ONE-SHOT INITIAL SCREENING MECHANISM ===
        # One-shot initial screening as soon as IV is ready
        if (not self.initial_screen_done 
            and not self.IsWarmingUp 
            and len(self.iv_cache) >= 20):
            
            self.Log(f"[INITIAL_SCREEN] First IV cache ready with {len(self.iv_cache)} stocks. Running initial screening now.")
            # 1. Set last_screening_date so WeeklyScreeningAndTrading passes its first check
            self.last_screening_date = self.Time.date()
            self.screening_completed_today = False # Ensure it runs
            # 2. Call the screening logic
            self.WeeklyScreeningAndTrading(data)
            # 3. Set the flag to True to ensure this block NEVER runs again
            self.initial_screen_done = True
        # === END OF ONE-SHOT MECHANISM ===
    ...
    def PerformWeeklyScreening(self):
        ...
        """
        self.last_screening_date = self.Time.date()
        self.screening_completed_today = False # Reset flag to allow screening this week
    ...
    def WeeklyScreeningAndTrading(self, data):
        ...
        # Prevent running multiple times on the same day (stops memory crash)
        if self.screening_completed_today:
            return
        
        # === NEW GATE: Ensure we have IV data ready ===
        if not self.iv_cache or len(self.iv_cache) == 0:
            self.Log("[GATE] IV cache is empty. Skipping screening.")
            return

        if len(self.iv_cache) < 20:
            self.Log(f"[GATE] Not enough IV data yet. Have {len(self.iv_cache)}, need 20. Skipping screening.")
            return

        self.Log(f"[GATE_PASSED] IV cache ready with {len(self.iv_cache)} stocks. Proceeding with screening.")
        ...
        try:
            ...
            # === Frequency Check (14 Days) ===
            if self.last_trade_execution_date is not None and (self.Time.date() - self.last_trade_execution_date).days < self.screening_frequency_days:
                return
            ...
            self.last_trade_execution_date = self.Time.date()
        except Exception as e:
            self.Log(f"Error in WeeklyScreeningAndTrading: {str(e)}")
        finally:
            # Ensure we mark screening as completed for the day regardless of success/failure
            # to prevent infinite loops in OnData
            self.screening_completed_today = True
                
    def ReadIVFromData(self, data):
        """Extract IV directly from data.OptionChains every tick"""
        if not data.OptionChains:
            return

        for chain in data.OptionChains.Values:
            # Get the underlying equity symbol
            equity_symbol = chain.Symbol.Underlying
            
            # Skip if not in our active universe or is an ETF/Index we exclude
            if equity_symbol not in self.ActiveSecurities or equity_symbol in [self.spy, self.xlu, self.vix]:
                continue
            ...
            front_month = min(c.Expiry for c in contracts)
            front_contracts = [c for c in contracts if c.Expiry == front_month]
            
            if not front_contracts:
                continue
            
            spot = front_contracts[0].UnderlyingLastPrice
            
            # Find ATM
            atm_call = min([c for c in front_contracts if c.Right == OptionRight.Call], 
                           key=lambda c: abs(c.Strike - spot), default=None)
            atm_put = min([c for c in front_contracts if c.Right == OptionRight.Put], 
                          key=lambda c: abs(c.Strike - spot), default=None)
            
            if atm_call and atm_put:
                # Extract IV directly from the contract object (no indicator needed!)
                iv_value = atm_call.ImpliedVolatility
                
                if iv_value and iv_value > 0:
                    self.iv_cache[equity_symbol] = iv_value
    ...
    def ManagePositions(self, data):
        ...
        for order_ticket in self.Transactions.GetOpenOrderTickets():
            try:
                ...
                # Ensure both are offset-naive to avoid "can't subtract offset-naive and offset-aware" error
                ticket_time = order_ticket.Time.replace(tzinfo=None)
                current_time = self.Time.replace(tzinfo=None)
                time_open = (current_time - ticket_time).total_seconds()
                if time_open > 125:
